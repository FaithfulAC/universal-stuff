...
local bytecode = getscriptbytecode(PathOfScript)
...
local rOpList = {}
for i,v in next, opList do 
	rOpList[bit32.band((i - 1) * 227, 0xFF)] = v 
end 
...
local function luau_validatesettings(luau_settings)
	...
	assert(type(luau_settings.robloxEncoding) == "boolean", "luau_settings.robloxEncoding should be a boolean")
end
...
local function luau_deserialize(bytecode, luau_settings)
	...
	local function readInstruction(codeList)
		...
		local opinfo = luau_settings.robloxEncoding and rOpList[opcode] or opList[opcode + 1]
		...
		local inst = {
			opcode = luau_settings.robloxEncoding and table.find(opList, opinfo) - 1 or opcode;
			opname = opname;
			opmode = opmode;
			kmode = kmode;
			usesAux = usesAux;
